...
function spotifyTrackPlayerHtml(id,title,artist='',artwork='',extraClass='',options={}){
  ...
  const inferredArtist=row&&A[row[0]]?A[row[0]][0]:'';
  const player=window.SpotifyRadarPlayer;
  if(player&&typeof player.html==='function') return player.html({
    spotifyId,
    title:title||row&&row[1]||'Track',
...
function hydrateSpotifyRadarPlayerCovers(root){
  ...
  [...root.querySelectorAll('[data-spotify-player-cover-id]')]
    .filter(node=>!node.dataset.spotifyPlayerCoverHydrated)
    .slice(0,8)
    .forEach(node=>{
      node.dataset.spotifyPlayerCoverHydrated='1';
      const id=spotifyTrackId(node.dataset.spotifyPlayerCoverId);if(!id)return;
      spotifyThumbnailWhenVisible(node,spotifyTrackUrl(id),imageUrl=>{
        const playerNode=node.closest('.spotify-radar-player');
        if(playerNode&&window.SpotifyRadarPlayer&&imageUrl)window.SpotifyRadarPlayer.setCover(playerNode,imageUrl);
      });
    });
}
function mountSpotifyRadarPlayers(root){
  if(window.SpotifyRadarPlayer)window.SpotifyRadarPlayer.hydrate(root);
  hydrateSpotifyRadarPlayerCovers(root);
}
...
function arListHas(spotifyId){return Boolean(arListEntry(spotifyId))&&arSelectionEligible(spotifyId);}
function arListCount(){return Object.keys(arListGet()).filter(arListHas).length;}
...
function arAddToList(spotifyId,event){
  ...
  arAddManyToList([spotifyId]);
  const prompted=arPromptArtistCompanions(spotifyId);
  const modal=document.getElementById('ar-modal');
  if(!prompted&&modal&&modal.style.display==='flex')openArOpportunity(spotifyId);
}
function arStructuredArtistIds(opportunity){
  return new Set((Array.isArray(opportunity&&opportunity.artists)?opportunity.artists:[]).map(artist=>String(artist&&artist.spotify_id||'').trim()).filter(Boolean));
}
function arArtistCompanionRows(spotifyId){
  const source=arOpportunityRows().find(item=>item.spotifyId===spotifyId);
  const artistIds=arStructuredArtistIds(source);
...
function arOutreachDrafts(opportunity){
  ...
  return [
    {label:'Découverte',subject:`About “${opportunity.title}”`,body:`Hi ${artist},\n\nWe discovered “${opportunity.title}” through ${playlist} while reviewing independent instrumental releases. The track stood out for its ${arGenreLabel(opportunity.genre)} direction and current audience signals.\n\nWe would love to learn more about your plans for the project and see whether a conversation could be useful. Would you be open to a short introduction?\n\nBest,\n[Your name]`},
    {label:'Signal éditorial',subject:`A note after hearing “${opportunity.title}”`,body:`Hi ${artist},\n\n“${opportunity.title}” came to our attention through ${playlist}. We liked the clarity of its ${arGenreLabel(opportunity.genre)} world and wanted to reach out directly.\n\nWould you be open to telling us a little about the project and what you are building next?\n\nBest,\n[Your name]`},
    {label:'Projet artiste',subject:`Getting to know your project`,body:`Hi ${artist},\n\nI am writing after discovering “${opportunity.title}”. Beyond this release, we would love to understand the wider direction of your artist project.\n\nAt Lofi Girl, we work with independent instrumental artists across label, distribution and publishing. If a short introduction feels relevant, I would be glad to set one up.\n\nBest,\n[Your name]`},
    {label:'Partenariat',subject:`A quick note about “${opportunity.title}”`,body:`Hi ${artist},\n\nI’m reaching out after spending time with “${opportunity.title}”. Its ${arGenreLabel(opportunity.genre)} direction caught our attention while we were mapping independent instrumental projects.\n\nWe are always interested in learning how artists want to develop their next releases and whether our label, distribution or publishing teams could be useful. Would you be open to a short conversation?\n\nBest,\n[Your name]`},
    {label:'Très court',subject:`Loved “${opportunity.title}”`,body:`Hi ${artist},\n\nA quick note after discovering “${opportunity.title}” via ${playlist}: we really connected with its ${arGenreLabel(opportunity.genre)} identity.\n\nOpen to a short introduction with the Lofi Girl team?\n\nBest,\n[Your name]`},
    {label:'Prochaine sortie',subject:`Your next releases`,body:`Hi ${artist},\n\nWe came across “${opportunity.title}” and wanted to introduce ourselves. We are especially curious about what you have planned next and how you would like to develop upcoming releases.\n\nIf useful, we would be happy to share how Lofi Girl supports independent artists.\n\nBest,\n[Your name]`},
    {label:'Catalogue',subject:`A conversation about your catalogue`,body:`Hi ${artist},\n\nWe have been reviewing independent instrumental catalogues and “${opportunity.title}” stood out to us.\n\nIf you are open to it, we would value a short conversation about your existing catalogue and upcoming work, with no commitment implied.\n\nBest,\n[Your name]`},
    {label:'Personnalisé',subject:`Really enjoyed “${opportunity.title}”`,body:`Hi ${artist},\n\nI wanted to reach out personally because the atmosphere and ${arGenreLabel(opportunity.genre)} direction of “${opportunity.title}” stayed with us after our review.\n\nWe would love to hear where you want to take the project next and introduce the Lofi Girl team if the timing is right.\n\nBest,\n[Your name]`}
  ];
}
function arOutreachDraft(opportunity){return arOutreachDrafts(opportunity)[0];}
function arMarkContacted(spotifyId){
  arUpdateList(spotifyId,{status:'contacted',contactedAt:new Date().toISOString(),nextFollowUp:arFollowUpDate()});
  const opportunity=arOpportunityRows().find(item=>item.spotifyId===spotifyId);
  if(opportunity)arMarkArtistContacted(arSelectionPrimaryArtist(opportunity).key);
  renderArList();
}
function arSelectionArtistInfoHtml(opportunity){
  const listeners=opportunity.artistMonthlyListeners==null?'':` · ${fmt(opportunity.artistMonthlyListeners)} auditeurs/mois`;
  return `<div class="ar-follow-artist">${arArtistLinksHtml(opportunity)}<span> · ${esc(arGenreLabel(opportunity.genre))}${listeners}</span></div><div class="ar-follow-contact ar-selection-contact">${arContactHtml(opportunity,true)}</div>`;
}
...
function renderRadarLegacy(){
  ...
  if(trackPool.length){
    const trackPanel=`<div class="panel ar-track-panel">
      ...
      ${selectedTrack?spotifyTrackPlayerHtml(selectedTrack.spotifyId,selectedTrack.title,selectedTrack.artist,arTrackCoverUrl(selectedTrack),'ar-player'):''}
    </div>`;
...
/* Selection is a review workspace, not a contact approval. Every track that
   is visible in Pistes can be retained here; outreach stays behind the strict
   Soundcharts eligibility checks in arContactEligible(). */
function arCatalogueSelectionOpportunity(spotifyId){
  const id=String(spotifyId||'').trim(),track=arTrackRowById(id); if(!track)return null;
  const artist=A[track[0]]||[],classification=track.scClassification||track.discoveryMeta||{};
  const artistName=String(track.scCredit||artist[0]||'Artiste non renseigné');
  const artistSpotifyId=String(artist[7]||'');
  return {
    selectionOnly:true,status:'catalogue',spotifyId:id,title:String(track[1]||'Titre non renseigné'),credit:artistName,
    artists:artistName?[{name:artistName,spotify_id:artistSpotifyId}]:[],imageUrl:String(track[8]||''),
...
function arSelectionOpportunityById(spotifyId){
  ...
  return arOpportunityRows().find(item=>item.spotifyId===id)||arCatalogueSelectionOpportunity(id);
}
...
function arPlatformLogo(platform){
  ...
  const color=({instagram:'E4405F',bandcamp:'1DA0C3',soundcloud:'FF5500',youtube:'FF0000',tiktok:'FFFFFF',facebook:'1877F2',twitter:'FFFFFF',x:'FFFFFF',spotify:'1ED760'})[platform]||'FFFFFF';
  return slug?`<img src="https://cdn.simpleicons.org/${slug}/${color}" alt="" loading="lazy">`:'';
}
...
function arEditorialPlaylistTooltip(playlist,opportunity){
  const name=playlist.name||playlist.spotifyId||'Playlist éditoriale';
  const position=playlist.position==null?'Position non observée':'Position #'+Math.round(playlist.position);
  const followers=playlist.followers==null||playlist.followers<=0?'Audience non observée':fmtFull(playlist.followers)+' followers';
  const since=arPlaylistSince(playlist.firstSeen||opportunity.firstPlaylistSeen);
...
function arReasonIcon(code,reason){
  ...
  if(/deal|right/.test(code)||/droit|self-release|indépendant/i.test(reason)) return '©️';
  if(code==='recent_release'||/sortie|récence/i.test(reason)) return '🗓️';
...
function arOpportunityMatchesSearch(opportunity,query){
  const terms=arSearchText(query).split(' ').filter(Boolean);
...
function openArOpportunity(spotifyId){
  const opportunity=arSelectionOpportunityById(spotifyId); if(!opportunity) return;
...
function hydrateArArtistAvatars(){
  if(typeof fetch!=='function') return;
  [...document.querySelectorAll('[data-ar-artist-avatar-id]')]
    .filter(node=>node.dataset.arArtistAvatarId&&!node.dataset.arArtistAvatarHydrated)
    .slice(0,40)
    .forEach(node=>{
      node.dataset.arArtistAvatarHydrated='1';
      const id=node.dataset.arArtistAvatarId;
      const apply=imageUrl=>{
        if(!imageUrl||!node.isConnected)return;
        const image=document.createElement('img');
        image.src=imageUrl;image.alt='';image.loading='lazy';
...
function arOpenSelectionArtistProfile(artistSpotifyId,fallbackTrackId){
  ...
  const index=track?track[0]:AG.findIndex(item=>String(item&&item.id||'')===String(artistSpotifyId||''));
  if(index>=0){goArtist(index);return;}
  const opportunity=arOpportunityRows().find(item=>item.spotifyId===fallbackTrackId);
  const name=opportunity?arSelectionPrimaryArtist(opportunity).name:'artiste';
  window.open(artistSpotifyId?spotifyArtistUrl(artistSpotifyId):spotifySearchUrl(name),'_blank','noopener');
}
...
function arSelectionArtistCardHtml(group){
  ...
  const artistName=`<button type="button" class="ar-selection-artist-name" onclick="arOpenSelectionArtistProfile('${esc(artist.spotifyId)}','${esc(contactOpportunity.spotifyId)}')">${esc(artist.name)}</button>`;
...
function arSelectionEconomics(group){
  ...
  const tracks=R.filter(track=>ids.has(String(track&&track[6]||'')));
...
function arSelectionGroupByArtistKey(artistKey){
  ...
  const rows=Object.keys(saved).map(id=>({opportunity:arSelectionOpportunityById(id),entry:saved[id]}))
    .filter(item=>item.opportunity);
  return arSelectionArtistGroups(rows).find(group=>group.artist.key===artistKey)||null;
}
function closeArSelectionEstimate(){
  const modal=document.getElementById('estimate-modal');
  if(modal) modal.style.display='none';
}
function openArSelectionEstimate(artistKey){
  const group=arSelectionGroupByArtistKey(artistKey); if(!group) return;
  const box=document.getElementById('estimate-body'); if(!box) return;
  box.innerHTML=`<div class="thd"><div class="av-sm">💶</div><div style="min-width:0;flex:1"><h3>Estimation interne</h3><div class="tar">${esc(group.artist.name)}</div></div><button class="tclose" onclick="closeArSelectionEstimate()">✕</button></div>${arSelectionEconomicsHtml(group)}`;
...
function renderOpps(){
  ...
  <div class="catalogue-list-panel">
    <div class="catalogue-table-wrap"><table class="catalogue-table">
      ...
      ${slice.map(r=>{ const w1=trackWindow(r,1), w7=trackWindow(r,7), w30=trackWindow(r,30); return `
        <tr data-ar-browse-track="${esc(r[6])}" data-basehot="${r[3]>=HOT?1:0}" class="${r[3]>=HOT||(ag&&S.sel.has(r[6]))?'hot':''}" onclick="openTrackFromCatalogueRow(event,'${r[6]}')">
  ...
  <div class="acards">
    ${slice.map(r=>{ const w1=trackWindow(r,1), w7=trackWindow(r,7), w30=trackWindow(r,30); return `
    <div class="acard plcard grid-clean" data-ar-browse-track="${esc(r[6])}" onclick="openTrack('${r[6]}')">
