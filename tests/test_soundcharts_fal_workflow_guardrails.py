...
class SoundchartsFalWorkflowGuardrailsTests(unittest.TestCase):
    ...
    def test_manual_start_and_cloud_resume_are_enabled(self):
        ...
        self.assertIn("- cron: '23 */6 * * *'", self.workflow)
    ...
    def test_checkpoint_is_private_separate_and_durable(self):
        ...
        self.assertIn("if: always()", self.workflow[state_upload:report_upload])
    ...
    def test_full_seed_cohort_and_protected_quota_reserve_are_explicit(self):
        self.assertIn("EXPECTED_SEEDS: '4186'", self.workflow)
        self.assertIn("MIN_SEEDS: '4186'", self.workflow)
        self.assertIn("MAX_SEEDS: '4186'", self.workflow)
        self.assertIn("QUOTA_RESERVE: '500000'", self.workflow)
        self.assertRegex(self.workflow, r"--max-seeds\s+[\"']?\$MAX_SEEDS")
        self.assertRegex(self.workflow, r"--min-seed-guard\s+[\"']?\$MIN_SEEDS")
        self.assertRegex(self.workflow, r"--max-seed-guard\s+[\"']?\$MAX_SEEDS")
        ...
        self.assertIn('report.get("cohort")', self.workflow)
        self.assertIn("if not minimum <= cohort_size <= maximum", self.workflow)
        self.assertEqual(self.workflow.count("--seed-min-audience 50000"), 2)
