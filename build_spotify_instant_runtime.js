...
const LABEL_LICENSE_RE = /(?:under\s+(?:an?\s+)?|on\s+)?exclusive\s+licen[cs]e\s+(?:to|with|from)\s+(.+?)(?:\s*[;.]|$)/i;
const LABEL_LICENSED_TO_RE = /licen[cs]ed\s+to\s+(.+?)(?:\s*[;.]|$)/i;
const LABEL_YEAR_STRIP_RE = /^[©℗()pcPC\s]*\d{4}\s*/;

function labelDisplayName(input) {
  const raw = text(input).trim();
  if (!raw) return '';
  const match = LABEL_LICENSE_RE.exec(raw) || LABEL_LICENSED_TO_RE.exec(raw);
  const extracted = match
    ? match[1].trim().replace(/\.$/, '')
    : raw.split(';')[0].trim().replace(LABEL_YEAR_STRIP_RE, '').trim();
  return extracted
...
function main() {
  ...
  const labelAnalyticsPath = argument('--label-analytics', path.join(ROOT, 'Spotify_Label_Analytics_data.js'));
  ...
  // Build the visible label catalogue from the same active track projection
  // as the rest of the lightweight Radar. The historical label export only
  // contributes curated logo/contact metadata; it must never reintroduce
  // retired labels or counters that no longer match the active catalogue.
  const curatedLabels = new Map();
  for (const row of labelsSource.rows) {
    if (!Array.isArray(row)) continue;
    const key = labelKeyOf(row[l.key]) || labelKeyOf(row[l.name]);
    if (!key || curatedLabels.has(key)) continue;
    curatedLabels.set(key, {logo: text(row[l.logo]), email: text(row[l.email])});
  }
  const activeLabelGroups = new Map();
  const labelPosition = trackSchema.indexOf('label');
  const copyrightPosition = trackSchema.indexOf('copyright');
  const streamsPosition = trackSchema.indexOf('streams');
  const releasePosition = trackSchema.indexOf('release_date');
  const artistIdPosition = trackSchema.indexOf('artist_spotify_id');
  const creditPosition = trackSchema.indexOf('credit_name');
  for (const row of tracks) {
    const source = text(row[labelPosition]).trim() || text(row[copyrightPosition]).trim();
    const key = labelKeyOf(source);
    if (!key) continue;
    let group = activeLabelGroups.get(key);
    if (!group) {
      group = {name: labelDisplayName(source) || key, tracks: 0, streams: 0, since: '', artists: new Set()};
      activeLabelGroups.set(key, group);
    }
    group.tracks += 1;
    group.streams += Math.max(0, finite(row[streamsPosition]) || 0);
    const release = text(row[releasePosition]).slice(0, 10);
    if (release && (!group.since || release < group.since)) group.since = release;
    const artist = text(row[artistIdPosition]).trim() || text(row[creditPosition]).trim().toLowerCase();
    if (artist) group.artists.add(artist);
  }
  const labels = [...activeLabelGroups.entries()].map(([key, group]) => {
    const curated = curatedLabels.get(key) || {};
    return [
      key, group.name, group.tracks, Math.round(group.streams), group.since,
      group.artists.size, curated.logo || '', curated.email || '',
    ];
  });
  ...
  // Label analytics is part of the same public projection and is rebuilt by
  // every existing daily/Pages invocation of this generator. Keeping this
  // dependency here prevents the visible catalogue and its click-through
  // metrics from ever drifting without expanding workflow orchestration.
  const labelAnalytics = require('./build_spotify_label_analytics.js').buildFile({
    cataloguePath,
    labelsPath: labelsLoaded.sourcePath,
    performancePath: performanceLoaded.sourcePath,
    outputPath: labelAnalyticsPath,
    minimumGroups: 100,
  });
  process.stdout.write(
    ...
    `${labelAnalytics.active_groups} label analytics groups through ${labelAnalytics.data_through || 'n/a'}.\n`,
  );
}
