...
test("shows native demographic dimensions below the chart and follows the selected platform", async () => {
  ...
  assert.doesNotMatch(explorer, /La source native ne fournit pas les rangs/);
  ...
  assert.match(explorer, /className="audience-demographic-pie-svg"/);
  assert.match(explorer, /shapeRendering="geometricPrecision"/);
  assert.match(explorer, /pathLength="1"/);
  assert.match(explorer, /strokeDasharray=\{stroke\.dashArray\}/);
  assert.match(explorer, /strokeDashoffset=\{stroke\.dashOffset\}/);
  assert.match(explorer, /strokeLinecap="butt"/);
  ...
  assert.match(explorer, /audienceDemographicPieStrokeGeometry\(slice, pieSlices\.length\)/);
  ...
  assert.match(explorer, /AUDIENCE_DEMOGRAPHIC_PIE_SEPARATOR_FRACTION = 0\.0028/);
  assert.match(explorer, /const gap = sliceCount > 1[\s\S]*?Math\.min\(AUDIENCE_DEMOGRAPHIC_PIE_SEPARATOR_FRACTION, span \* 0\.18\)/);
  assert.doesNotMatch(explorer, /conic-gradient/);
  assert.doesNotMatch(explorer, /pieGradient/);
