...
const expectedSources = new Map([
  ...
  ["DX60keuzeIU", { author: "cozy.moji", likes: 246_325 }],
  ["C6EHz5AL8wK", { author: "caimarsalizi", likes: 600_584 }],
  ["DTsQD2iDCaB", { author: "alice.journeyy", likes: 127_100, comments: 300 }],
]);
...
test("the connected Instagram snapshot preserves the historical and new-account runs", () => {
  ...
  assert.equal(feed.runs.length, 47);
  assert.equal(feed.items.length, 75);
  ...
  const septemberFifthReelsRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-05-heartbeat-1515",
  );
  const septemberFifthHomeRun = feed.runs.find(
    (run) => run.id === "instagram-home-2026-09-05-heartbeat-1515",
  );
  const septemberFifthEveningReelsRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-05-heartbeat-1915",
  );
  const septemberFifthEveningHomeRun = feed.runs.find(
    (run) => run.id === "instagram-home-2026-09-05-heartbeat-1915",
  );
  const septemberFifthLateReelsRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-05-heartbeat-2115",
  );
  const septemberFifthLateHomeRun = feed.runs.find(
    (run) => run.id === "instagram-home-2026-09-05-heartbeat-2115",
  );
  const septemberFifthNightReelsRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-05-heartbeat-2315",
  );
  ...
  const septemberSixthLateMorningReelsRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-06-heartbeat-1115",
  );
  const septemberSixthLateMorningHomeRun = feed.runs.find(
    (run) => run.id === "instagram-home-2026-09-06-heartbeat-1115",
  );
  const septemberSixthAfternoonReelsRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-06-heartbeat-1315",
  );
  ...
  const septemberSixthEveningReelsRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-06-heartbeat-1715",
  );
  const septemberSixthEveningHomeRun = feed.runs.find(
    (run) => run.id === "instagram-home-2026-09-06-heartbeat-1715",
  );
  const septemberSixthLateEveningReelsRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-06-heartbeat-1915",
  );
  const septemberSixthLateEveningHomeRun = feed.runs.find(
    (run) => run.id === "instagram-home-2026-09-06-heartbeat-1915",
  );
  const septemberSixthNightRun = feed.runs.find(
    (run) => run.id === "instagram-reels-2026-09-06-heartbeat-2115",
  );
  ...
  assert.ok(septemberFifthReelsRun);
  assert.ok(septemberFifthHomeRun);
  assert.ok(septemberFifthEveningReelsRun);
  assert.ok(septemberFifthEveningHomeRun);
  assert.ok(septemberFifthLateReelsRun);
  ...
  assert.ok(septemberSixthLateMorningReelsRun);
  ...
  assert.ok(septemberSixthEveningReelsRun);
  assert.ok(septemberSixthEveningHomeRun);
  assert.ok(septemberSixthLateEveningReelsRun);
  assert.ok(septemberSixthLateEveningHomeRun);
  assert.ok(septemberSixthNightRun);
  ...
  assert.equal(septemberFifthReelsRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberFifthReelsRun.surface, "reels");
  assert.equal(septemberFifthReelsRun.seenCount, 143);
  assert.equal(septemberFifthReelsRun.qualifyingCount, 0);
  assert.equal(septemberFifthReelsRun.sponsoredCount, 0);
  assert.equal(feed.items.filter((item) => item.runId === septemberFifthReelsRun.id).length, 0);
  assert.ok(septemberFifthReelsRun.themeIds.includes("releases-artist-collabs"));
  assert.match(septemberFifthReelsRun.limitations.join(" "), /quatre.*(?:Paid partnership|#ad)/iu);
  assert.equal(septemberFifthHomeRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberFifthHomeRun.surface, "home");
  assert.equal(septemberFifthHomeRun.seenCount, 89);
  assert.equal(septemberFifthHomeRun.qualifyingCount, 0);
  assert.equal(septemberFifthHomeRun.sponsoredCount, 0);
  assert.equal(feed.items.filter((item) => item.runId === septemberFifthHomeRun.id).length, 0);
  assert.equal(septemberFifthReelsRun.seenCount + septemberFifthHomeRun.seenCount, 232);
  assert.equal(septemberFifthReelsRun.sponsoredCount + septemberFifthHomeRun.sponsoredCount, 0);
  assert.ok(
    Date.parse(septemberFifthReelsRun.capturedAt) >
      Date.parse(septemberFourthLateHomeRun.capturedAt),
  );
  assert.equal(septemberFifthReelsRun.capturedAt, septemberFifthHomeRun.capturedAt);
  assert.equal(septemberFifthEveningReelsRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberFifthEveningReelsRun.surface, "reels");
  assert.equal(septemberFifthEveningReelsRun.seenCount, 153);
  assert.equal(septemberFifthEveningReelsRun.qualifyingCount, 0);
  assert.equal(septemberFifthEveningReelsRun.sponsoredCount, 0);
  assert.ok(septemberFifthEveningReelsRun.themeIds.includes("reading-and-journaling"));
  assert.equal(septemberFifthEveningHomeRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberFifthEveningHomeRun.surface, "home");
  assert.equal(septemberFifthEveningHomeRun.seenCount, 85);
  assert.equal(septemberFifthEveningHomeRun.qualifyingCount, 0);
  assert.equal(septemberFifthEveningHomeRun.sponsoredCount, 0);
  assert.equal(
    septemberFifthEveningReelsRun.seenCount + septemberFifthEveningHomeRun.seenCount,
    238,
  );
  assert.ok(
    Date.parse(septemberFifthEveningReelsRun.capturedAt) >
      Date.parse(septemberFifthHomeRun.capturedAt),
  );
  assert.equal(septemberFifthEveningReelsRun.capturedAt, septemberFifthEveningHomeRun.capturedAt);
  assert.equal(septemberFifthLateReelsRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberFifthLateReelsRun.surface, "reels");
  assert.equal(septemberFifthLateReelsRun.seenCount, 153);
  assert.equal(septemberFifthLateReelsRun.qualifyingCount, 0);
  assert.equal(septemberFifthLateReelsRun.sponsoredCount, 0);
  assert.match(septemberFifthLateReelsRun.limitations.join(" "), /59 vidéos.*muettes/iu);
  assert.equal(septemberFifthLateHomeRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberFifthLateHomeRun.surface, "home");
  assert.equal(septemberFifthLateHomeRun.seenCount, 84);
  assert.equal(septemberFifthLateHomeRun.qualifyingCount, 0);
  assert.equal(septemberFifthLateHomeRun.sponsoredCount, 0);
  assert.equal(septemberFifthLateReelsRun.seenCount + septemberFifthLateHomeRun.seenCount, 237);
  assert.match(
    septemberFifthLateHomeRun.limitations.join(" "),
    /relevantObserved=28.*offNicheObserved=33.*relevanceRate=45,9 %/u,
  );
  assert.ok(
    Date.parse(septemberFifthLateReelsRun.capturedAt) >
      Date.parse(septemberFifthEveningHomeRun.capturedAt),
  );
  assert.equal(septemberFifthLateReelsRun.capturedAt, septemberFifthLateHomeRun.capturedAt);
  assert.equal(septemberFifthNightReelsRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberFifthNightReelsRun.surface, "reels");
  assert.equal(septemberFifthNightReelsRun.seenCount, 137);
  assert.equal(septemberFifthNightReelsRun.qualifyingCount, 0);
  assert.equal(septemberFifthNightReelsRun.sponsoredCount, 0);
  assert.match(septemberFifthNightReelsRun.limitations.join(" "), /60 vidéos.*muettes/iu);
  assert.equal(septemberFifthNightHomeRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberFifthNightHomeRun.surface, "home");
  assert.equal(septemberFifthNightHomeRun.seenCount, 77);
  assert.equal(septemberFifthNightHomeRun.qualifyingCount, 0);
  assert.equal(septemberFifthNightHomeRun.sponsoredCount, 0);
  assert.equal(septemberFifthNightReelsRun.seenCount + septemberFifthNightHomeRun.seenCount, 214);
  assert.match(
    septemberFifthNightHomeRun.limitations.join(" "),
    /relevantObserved=36.*offNicheObserved=22.*relevanceRate=62,1 %/u,
  );
  assert.ok(
    Date.parse(septemberFifthNightReelsRun.capturedAt) >
  ...
  assert.equal(
    septemberSixthLateMorningReelsRun.browserContext,
    SCROLLING_AGENT_TAB_CONTEXT,
  );
  assert.equal(septemberSixthLateMorningReelsRun.surface, "reels");
  assert.equal(septemberSixthLateMorningReelsRun.seenCount, 137);
  assert.equal(septemberSixthLateMorningReelsRun.qualifyingCount, 0);
  assert.equal(septemberSixthLateMorningReelsRun.sponsoredCount, 0);
  assert.ok(septemberSixthLateMorningReelsRun.themeIds.includes("animal-companions"));
  assert.match(
    septemberSixthLateMorningReelsRun.limitations.join(" "),
    /study timer.*cozy room.*embed Instagram officiel/iu,
  );
  assert.equal(
    septemberSixthLateMorningHomeRun.browserContext,
    SCROLLING_AGENT_TAB_CONTEXT,
  );
  assert.equal(septemberSixthLateMorningHomeRun.surface, "home");
  assert.equal(septemberSixthLateMorningHomeRun.seenCount, 76);
  assert.equal(septemberSixthLateMorningHomeRun.qualifyingCount, 0);
  assert.equal(septemberSixthLateMorningHomeRun.sponsoredCount, 0);
  assert.equal(
    septemberSixthLateMorningReelsRun.seenCount + septemberSixthLateMorningHomeRun.seenCount,
    213,
  );
  assert.match(
    septemberSixthLateMorningHomeRun.limitations.join(" "),
    /relevantObserved=33.*offNicheObserved=26.*relevanceRate=55,9 %/u,
  );
  assert.match(
    septemberSixthLateMorningHomeRun.limitations.join(" "),
    /WhatsApp.*Oakley Meta.*publicités natives/iu,
  );
  assert.ok(
    Date.parse(septemberSixthLateMorningReelsRun.capturedAt) >
      Date.parse(septemberSixthMorningHomeRun.capturedAt),
  );
  assert.equal(
    septemberSixthLateMorningReelsRun.capturedAt,
    septemberSixthLateMorningHomeRun.capturedAt,
  );
  assert.equal(septemberSixthAfternoonReelsRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberSixthAfternoonReelsRun.surface, "reels");
  assert.equal(septemberSixthAfternoonReelsRun.seenCount, 137);
  assert.equal(septemberSixthAfternoonReelsRun.qualifyingCount, 0);
  assert.equal(septemberSixthAfternoonReelsRun.sponsoredCount, 0);
  assert.equal(septemberSixthAfternoonHomeRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberSixthAfternoonHomeRun.surface, "home");
  assert.equal(septemberSixthAfternoonHomeRun.seenCount, 76);
  assert.equal(septemberSixthAfternoonHomeRun.qualifyingCount, 0);
  assert.equal(septemberSixthAfternoonHomeRun.sponsoredCount, 0);
  assert.equal(
    septemberSixthAfternoonReelsRun.seenCount + septemberSixthAfternoonHomeRun.seenCount,
    213,
  );
  assert.match(
    septemberSixthAfternoonHomeRun.limitations.join(" "),
    /relevantObserved=36.*offNicheObserved=24.*relevanceRate=60,0 %/u,
  );
  assert.match(
    septemberSixthAfternoonHomeRun.limitations.join(" "),
    /DcwC3vbE9yc.*#ad.*Tom Ford.*YSL.*publicités natives/iu,
  );
  assert.ok(
    Date.parse(septemberSixthAfternoonReelsRun.capturedAt) >
      Date.parse(septemberSixthLateMorningHomeRun.capturedAt),
  );
  assert.equal(
  ...
  assert.equal(septemberSixthEveningReelsRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberSixthEveningReelsRun.surface, "reels");
  assert.equal(septemberSixthEveningReelsRun.seenCount, 137);
  assert.equal(septemberSixthEveningReelsRun.qualifyingCount, 0);
  assert.equal(septemberSixthEveningReelsRun.sponsoredCount, 0);
  assert.match(
    septemberSixthEveningReelsRun.limitations.join(" "),
    /2,5 secondes.*dwell.*57 contrôles/iu,
  );
  assert.equal(septemberSixthEveningHomeRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberSixthEveningHomeRun.surface, "home");
  assert.equal(septemberSixthEveningHomeRun.seenCount, 79);
  assert.equal(septemberSixthEveningHomeRun.qualifyingCount, 0);
  assert.equal(septemberSixthEveningHomeRun.sponsoredCount, 0);
  assert.equal(
    septemberSixthEveningReelsRun.seenCount + septemberSixthEveningHomeRun.seenCount,
    216,
  );
  assert.match(
    septemberSixthEveningHomeRun.limitations.join(" "),
    /relevantObserved=53.*offNicheObserved=19.*relevanceRate=73,6 %/u,
  );
  assert.match(
    septemberSixthEveningHomeRun.limitations.join(" "),
    /Ulla Johnson.*Dover Street Market.*Manderley Press.*Dc1x4_tI5RC.*CapCut AI/iu,
  );
  assert.ok(
    Date.parse(septemberSixthEveningReelsRun.capturedAt) >
      Date.parse(septemberSixthLateAfternoonHomeRun.capturedAt),
  );
  assert.equal(septemberSixthEveningReelsRun.capturedAt, septemberSixthEveningHomeRun.capturedAt);
  assert.equal(
    septemberSixthLateEveningReelsRun.browserContext,
    SCROLLING_AGENT_TAB_CONTEXT,
  );
  assert.equal(septemberSixthLateEveningReelsRun.surface, "reels");
  assert.equal(septemberSixthLateEveningReelsRun.seenCount, 167);
  assert.equal(septemberSixthLateEveningReelsRun.qualifyingCount, 2);
  assert.equal(septemberSixthLateEveningReelsRun.sponsoredCount, 0);
  assert.equal(
    feed.items.filter((item) => item.runId === septemberSixthLateEveningReelsRun.id).length,
    2,
  );
  assert.match(
    septemberSixthLateEveningReelsRun.limitations.join(" "),
    /2,5 secondes.*52 contrôles.*Deux mécaniques/iu,
  );
  assert.equal(
    septemberSixthLateEveningHomeRun.browserContext,
    SCROLLING_AGENT_TAB_CONTEXT,
  );
  assert.equal(septemberSixthLateEveningHomeRun.surface, "home");
  assert.equal(septemberSixthLateEveningHomeRun.seenCount, 75);
  assert.equal(septemberSixthLateEveningHomeRun.qualifyingCount, 0);
  assert.equal(septemberSixthLateEveningHomeRun.sponsoredCount, 0);
  assert.equal(
    septemberSixthLateEveningReelsRun.seenCount +
      septemberSixthLateEveningHomeRun.seenCount,
    242,
  );
  assert.match(
    septemberSixthLateEveningHomeRun.limitations.join(" "),
    /relevantObserved=54.*offNicheObserved=11.*relevanceRate=83,1 %/u,
  );
  assert.match(
    septemberSixthLateEveningHomeRun.limitations.join(" "),
    /Paid partnership.*Carling.*Predator Energy.*Zeiss Nature/iu,
  );
  assert.ok(
    Date.parse(septemberSixthLateEveningReelsRun.capturedAt) >
      Date.parse(septemberSixthEveningHomeRun.capturedAt),
  );
  assert.equal(
    septemberSixthLateEveningReelsRun.capturedAt,
    septemberSixthLateEveningHomeRun.capturedAt,
  );
  assert.equal(septemberSixthNightRun.browserContext, SCROLLING_AGENT_TAB_CONTEXT);
  assert.equal(septemberSixthNightRun.surface, "reels");
  assert.equal(septemberSixthNightRun.seenCount, 236);
  assert.equal(septemberSixthNightRun.qualifyingCount, 5);
  assert.equal(septemberSixthNightRun.sponsoredCount, 0);
  assert.equal(feed.items.filter((item) => item.runId === septemberSixthNightRun.id).length, 1);
  assert.match(
    septemberSixthNightRun.limitations.join(" "),
    /161 observations ciblées.*75 Home.*68,2 %.*Quarante-huit publications ciblées.*sept seeds.*onze références/iu,
  );
  assert.match(
    septemberSixthNightRun.limitations.join(" "),
    /relevantObserved=45.*offNicheObserved=12.*relevanceRate=78,9 %.*Huit sources.*AI content/iu,
  );
  assert.ok(
    Date.parse(septemberSixthNightRun.capturedAt) >
      Date.parse(septemberSixthLateEveningHomeRun.capturedAt),
  );
  assert.equal(feed.capturedAt, septemberSixthNightRun.capturedAt);
  assert.match(feed.methodology, /47 runs et 75 cartes/u);
