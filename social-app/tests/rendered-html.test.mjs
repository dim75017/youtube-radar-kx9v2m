...
test("keeps real social collection, post formats and persistence explicit", async () => {
  ...
  assert.match(topRankingView, /id="top-duration-filter"[\s\S]*?label="Date de publication"/);
  ...
  assert.match(styles, /\.category-results-header\.category-results-toolbar\s*\{[^}]*display:\s*grid[^}]*grid-template-columns:\s*minmax\(360px, 500px\) minmax\(160px, 1fr\)/);
  assert.match(styles, /\.category-results-adjacent-filters\s*\{[^}]*grid-template-columns:\s*minmax\(190px, 1\.1fr\) minmax\(170px, 0\.9fr\)/);
  assert.match(styles, /\.category-results-sort-filter\s*\{[^}]*width:\s*min\(100%, 190px\)[^}]*justify-self:\s*end/);
  assert.match(styles, /@media \(max-width: 1180px\)[\s\S]*?\.category-results-header\.category-results-toolbar\s*\{[^}]*grid-template-columns:\s*minmax\(340px, 1\.4fr\) minmax\(150px, 0\.6fr\)/);
  ...
  const categoryResultsStart = topRankingView.indexOf("category-results");
  const adjacentFiltersStart = topRankingView.indexOf("category-results-adjacent-filters", categoryResultsStart);
  const categoryFilterStart = topRankingView.indexOf('id="top-format-filter"', adjacentFiltersStart);
  const durationFilterStart = topRankingView.indexOf('id="top-duration-filter"', categoryFilterStart);
  const sortFilterStart = topRankingView.indexOf('id="top-sort-filter"', durationFilterStart);
  const categoryHeaderEnd = topRankingView.indexOf("</header>", sortFilterStart);
  assert.ok(adjacentFiltersStart > categoryResultsStart);
  assert.ok(categoryFilterStart > adjacentFiltersStart);
  assert.ok(durationFilterStart > categoryFilterStart);
  assert.ok(sortFilterStart > durationFilterStart);
  assert.ok(categoryHeaderEnd > sortFilterStart);
  assert.doesNotMatch(topRankingView, /Catégorie active|category-results-heading|active-category-title/);
  assert.doesNotMatch(topRankingView, /top-ranking-dropdowns|category-results-filters/);
  assert.match(component, /<span className="section-kicker">Date de publication<\/span>/);
  ...
  assert.match(styles, /\.authored-comment-grid\s*\{[\s\S]*?grid-template-columns:\s*repeat\(auto-fill,\s*minmax\(340px,\s*1fr\)\)/);
  ...
  assert.match(styles, /\.authored-comment-target-strip\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\) auto/);
  assert.match(styles, /\.authored-comment-target-summary\s*\{[\s\S]*?display:\s*flex;/);
  assert.match(styles, /\.authored-comment-body blockquote\s*\{[\s\S]*?grid-template-rows:\s*minmax\(0,\s*1fr\) auto/);
  assert.match(styles, /\.authored-comment-body blockquote > p\s*\{[\s\S]*?overflow:\s*hidden;[\s\S]*?-webkit-line-clamp:\s*4/);
  assert.match(styles, /\.authored-comment-metrics\s*\{[\s\S]*?grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/);
  assert.doesNotMatch(styles, /\.authored-comment-target-account|\.authored-comment-target-identity|\.authored-comment-target-facts|\.authored-comment-body footer/);
