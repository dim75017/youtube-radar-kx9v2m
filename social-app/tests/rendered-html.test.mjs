...
test("keeps real social collection, post formats and persistence explicit", async () => {
  ...
  assert.match(styles, /\.category-results-header\.category-results-toolbar\s*\{[^}]*display:\s*grid[^}]*grid-template-columns:\s*max-content minmax\(340px, 500px\) minmax\(160px, 1fr\)/);
  assert.match(styles, /\.category-results-adjacent-filters\s*\{[^}]*grid-template-columns:\s*minmax\(170px, 0\.9fr\) minmax\(190px, 1\.1fr\)/);
  assert.match(styles, /\.category-results-sort-filter\s*\{[^}]*width:\s*min\(100%, 190px\)[^}]*justify-self:\s*end/);
  assert.match(styles, /@media \(max-width: 1180px\)[\s\S]*?\.category-results-header\.category-results-toolbar\s*\{[^}]*grid-template-columns:\s*minmax\(200px, 0\.8fr\) minmax\(320px, 1\.4fr\) minmax\(150px, 0\.6fr\)/);
  ...
  const adjacentFiltersStart = topRankingView.indexOf("category-results-adjacent-filters", categoryTitleStart);
  const durationFilterStart = topRankingView.indexOf('id="top-duration-filter"', adjacentFiltersStart);
  const categoryFilterStart = topRankingView.indexOf('id="top-format-filter"', durationFilterStart);
  const sortFilterStart = topRankingView.indexOf('id="top-sort-filter"', categoryFilterStart);
  const categoryHeaderEnd = topRankingView.indexOf("</header>", sortFilterStart);
  assert.ok(categoryTitleStart > categoryResultsStart);
  assert.ok(adjacentFiltersStart > categoryTitleStart);
  assert.ok(durationFilterStart > adjacentFiltersStart);
  assert.ok(categoryFilterStart > durationFilterStart);
  assert.ok(sortFilterStart > categoryFilterStart);
