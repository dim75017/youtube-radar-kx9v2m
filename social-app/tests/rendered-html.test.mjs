...
test("keeps real social collection, post formats and persistence explicit", async () => {
  ...
  assert.match(styles, /\.category-results-filters\s*\{[^}]*width:\s*min\(55vw,\s*620px\)[^}]*max-width:\s*620px[^}]*margin-right:\s*0[^}]*margin-left:\s*auto/);
  ...
  const categoryFiltersStart = topRankingView.indexOf("top-ranking-dropdowns category-results-filters", categoryTitleStart);
  const categoryHeaderEnd = topRankingView.indexOf("</header>", categoryFiltersStart);
  assert.doesNotMatch(topRankingView.slice(0, categoryResultsStart), /top-ranking-dropdowns/);
  assert.ok(categoryTitleStart > categoryResultsStart);
  assert.ok(categoryFiltersStart > categoryTitleStart);
  assert.ok(categoryHeaderEnd > categoryFiltersStart);
